...
import { SessionStatus, Message, ChatMode, UserProfile, GroundingSource, EmotionState } from '../types/serenity.ts';
...
- **安全**: 若涉及自残风险，请温和提示寻求专业热线。`;
  ...
  return (
    <div className={`flex flex-col h-full max-w-7xl mx-auto shadow-2xl relative overflow-hidden transition-all duration-[3000ms] ${getBackgroundClass()}`}>
      <div className="absolute inset-0 -z-10 transition-all duration-[3000ms]" style={{ backgroundColor: 'var(--bg-color-1, #f0f4ff)' }}>
        <div className="absolute w-[400px] h-[400px] -top-20 -left-20 rounded-full blur-[80px] opacity-40 transition-all duration-[5000ms] animate-breathe" style={{ backgroundColor: 'var(--blob-1, #e0f2fe)' }}></div>
        <div className="absolute w-[300px] h-[300px] bottom-0 -right-10 rounded-full blur-[80px] opacity-40 transition-all duration-[5000ms] animate-breathe" style={{ backgroundColor: 'var(--blob-2, #bae6fd)', animationDelay: '2s' }}></div>
      </div>
      ...
      {errorMessage && (
        <div className="mx-4 md:mx-6 mb-1.5 px-3 py-2 bg-rose-50/80 backdrop-blur-md text-rose-700 text-sm font-medium flex items-center justify-between rounded-xl border border-rose-100 shadow-sm animate-in fade-in slide-in-from-top-2 flex-shrink-0">
          ...
          <button onClick={() => setErrorMessage(null)} className="hover:bg-rose-100 rounded-full p-1 transition-colors flex-shrink-0 ml-2">
        ...
        {mode === ChatMode.VOICE ? (
          <div className="flex flex-col items-center space-y-3">
            ...
            {status === SessionStatus.CONNECTED ? (
              <button onClick={stopVoiceSession} className="w-full py-3 bg-rose-50/80 hover:bg-rose-100 text-rose-600 font-bold rounded-2xl border border-rose-200 transition-all flex items-center justify-center space-x-2 active:scale-95">
